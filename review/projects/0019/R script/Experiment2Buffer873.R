...
dat <- read.csv("dxl2015-el6383-project/output041_2.txt")
...
dat$trial <- 1
LFN1 <- dat
dat <- read.csv("dxl2015-el6383-project/output042_2.txt")
names(dat) <- cnames
dat$tcp <- "vegas"
dat$capacity <- 2
dat$rtt <- 2
dat$trial <- 2
LFN1 <- rbind(LFN1, dat)
dat <- read.csv("dxl2015-el6383-project/output043_2.txt")
names(dat) <- cnames
dat$tcp <- "vegas"
dat$capacity <- 2
dat$rtt <- 2
...
dat <- read.csv("dxl2015-el6383-project/output151_2.txt")
names(dat) <- cnames
dat$tcp <- "reno"
dat$capacity <- 2
dat$rtt <- 2
dat$trial <- 1
LFN1 <- rbind(LFN1, dat)
dat <- read.csv("dxl2015-el6383-project/output152_2.txt")
names(dat) <- cnames
dat$tcp <- "reno"
dat$capacity <- 2
dat$rtt <- 2
dat$trial <- 2
LFN1 <- rbind(LFN1, dat)
dat <- read.csv("dxl2015-el6383-project/output153_2.txt")
names(dat) <- cnames
dat$tcp <- "reno"
dat$capacity <- 2
dat$rtt <- 2
...
q <- q + ggtitle("Lineplot of Throughput Vegas vs.Reno in Buffer 873BYTE")
q <- q + geom_point(aes(x=interval.begin, y=tput, colour=tcp))
q <- q + geom_line(aes(x=interval.begin, y=tput, colour=tcp, linetype=trial))
q
ggsave("Exper2_lineplot.png", plot=q)
q <- ggplot(details)
q <- q + scale_x_discrete("interbal.begin")
q <- q + scale_y_continuous("Throughput (bps)")
q <- q + ggtitle("Violinplot of Throughput Vegas vs.Reno in Buffer 873BYTE")
q <- q + geom_violin(aes(x=tcp, y=tput))
q
ggsave("Exper2_violinplot.png", plot=q)
